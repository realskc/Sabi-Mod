...
- 价格解析器会缓存 `price(id)` 的结果来避免重复递归。
- 部分物品会出现在循环配方图里，第一次递归求值时可能被临时解析为空。
- 撒币机配置装载和价格解析 CLI 使用 `resolvedPrice(id)`；它会先查缓存价格，如果为空，再直接计算该物品自己的公式并取最低价。
- `resolvedPrice(id)` 直接计算成功后会把价格写回缓存，避免依赖它的后续物品继续读到早期空缓存。
- 这个 fallback 用于避免铜镐、铁镐等可直接由有价原料合成的物品因为早期递归缓存为空而被跳过。
